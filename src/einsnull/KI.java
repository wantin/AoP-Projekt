...
public class KI extends Spieler {
	 ...
	 * 
	 * Methode zur Generierung zufaelliger Einheiten, ein case repraesentiert einen Kartentyp
	 * @param Der zugehoerige Spieler fuer den die Einheit generiert werden soll, wichtig fuer Bewegungsrichtung links/rechts
	 * @return jeweilige zufaellige Karte wird zurueckgegegeben
	 */
	public Einheit generateEinheit() {
		Random zufall = new Random();
		int zufZahl = zufall.nextInt(4); 	// Zahl muss manuell je nach Anzahl der existierenden Klassen in 'einheitenkarten' geaendert werden
		switch (zufZahl) {					// case int AnzahlKarten: return new KartenTyp(this);
			...
			default:
				return null;	//sollte nicht vorkommen // TODO ach sag bloss...
		}
	}	
	...
	void ziehen(Feld spielbrett, GUI anzeige) {
		...
		if (!hand.isEmpty()) { //wenn Karten auf Hand, spiele sie
			...
			gespielt = true;
		} else {	//ansonsten ueberpruefe ob bereite Truppen auf Feld
		//	System.out.println("ueberpruefe ob bereite Truppen auf Feld");
			loop:
		...
		if (bereiteTruppen == true && gespielt == false) { //wenn bereite Truppen auf Feld, benutze diesen
			loop:
			// ueberpruefe ob angreifbare Einheit auf dem Feld, wenn ja greife an
			for (int i = 0; i < this.getTruppen().size(); i++) { 
				if (truppen.get(i).zeigeAngriff(spielbrett).size() > 0 && truppen.get(i).getBereit() == 1) {	//ueberpruefe eine Einheit ob angreifbare Einheit in Reichweite hat
					...
					gespielt = true;
					break loop;
				} else System.out.println(truppen.get(i).getName() + " kann nicht angreifen. Probiere n\u00f4chste Einheit.");
			}
		...
		if (bereiteTruppen == true && gespielt == false) {
			...
			System.out.print(truppen.get(auswahl).getName() + " :");
			truppen.get(auswahl).printBewegungen(spielbrett);
			System.out.println();System.out.println("________________________________________________________________________");
			...
			try {
				TimeUnit.MILLISECONDS.sleep(1421); //deal with it // TODO lol
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
