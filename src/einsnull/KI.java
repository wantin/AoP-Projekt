	...
	*/
	...
	void ziehen(Feld spielbrett, GUI anzeige){
		...
		if (!hand.isEmpty()) { //wenn Karten auf Hand, spiele sie
			...
			gespielt = true;
		} else { //ansonsten �berpr�fe ob bereite Truppen auf Feld
		//	System.out.println("�berpr�fe ob bereite Truppen auf Feld");
			loop:
		...
		if (bereiteTruppen == true && gespielt == false) { //wenn bereite Truppen auf Feld, benutze diese
			loop:
			//�berpr�fe ob angreifbare Einheit auf dem Feld, wenn ja greife an
			for (int i = 0; i < this.getTruppen().size(); i++) { 
				if (truppen.get(i).zeigeAngriff(spielbrett).size() > 0 && truppen.get(i).getBereit() == 1) { //�berpr�fe eine Einheit ob angreifbare Einheit in Reichweite hat
					...
					System.out.println(truppen.get(i).zeigeAngriff(spielbrett).size() + " m�gliche Angriffe: ");truppen.get(i).printAngriffe(spielbrett); 
					...
					gespielt = true;
					break loop;
				} else System.out.println(truppen.get(i).getName() + " kann nicht angreifen. Probiere n�chste Einheit.");
