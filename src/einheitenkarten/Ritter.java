...
public class Ritter extends Einheit {
	...
	public Ritter(Spieler besitzer) {
		...
		for (int i = -2; i < 3; i++) {
			for (int j = -2; j < 3; j++) {
				int[] temp = {i*vor, j*vor}; //vor braucht es eigentlich nicht, da die Bewegung vollkommen symmetrisch ist. // TODO wieso issed dann drin?!
				bewegung.add(temp);
				angriff.add(temp);
			}
		}
	...
	public void angreifen(Feld spielbrett, int zeile, int spalte) {
		...
		for (int i = -1; i < 2; i++) {
			for (int j = -1; j < 2; j++) {
				if (zeile+i > -1 && zeile+i < 6 && spalte+j > -1 && spalte+j < 6) {		// TODO unter umstaenden bedingungen klammern/anders strukturieren
					if (!spielbrett.besetzt(zeile+i, spalte+j) || (position[0] == zeile+i && position[1] == spalte+j)) {	// das Feld darf von dem Ritter selbst belegt sein
